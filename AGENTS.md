...
- `scripts/vm100-pilot.py protocol-matrix --protocol nfs --idle-read` is the separate mounted NFS long-idle recovery probe. It writes and remotely drains one owned 64 MiB file, idles for 61 minutes, invalidates the NFS client's page cache, applies a 30-second fio deadline, verifies exact I/O and SHA-256, and requires the `zerofs_sftp_object_read_bytes_total` delta to cover at least the logical read bytes before reporting success. It neither deploys nor restarts the server and fails closed when server cache state cannot prove SFTP backend traffic. A successful receipt proves the long-idle mounted read completed on an otherwise isolated test export; it does not by itself prove a pool session rotated.

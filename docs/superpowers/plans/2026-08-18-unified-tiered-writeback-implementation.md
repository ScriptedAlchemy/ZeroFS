...
- [ ] Sparse/interleaved reclaim verification for a real maximum 256 MiB compacted segment with approximately 8,192 32 KiB frames uses 16 fixed 512-key batches across two views (32 scans total), scans at most 65,536 rows/64 MiB encoded bytes, has fully qualified nonzero-selection focused GREEN gates, and fails closed on row/byte exhaustion, scan/decode error, or live reference.

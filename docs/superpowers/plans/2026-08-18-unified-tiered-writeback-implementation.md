...
- [ ] Sparse/interleaved reclaim verification for a full 1,024-frame segment uses at most 16 fixed 64-key batches across two views (32 scans total), bounded streaming memory, no per-frame point-read fanout, and fail-closed error/reference handling.
...
- [ ] C7B records independent typed HPN-receive-win, upload-gap-detected, and upload-gap-resolved flags in one manifest-validated archived authority; C8 requires every detected gap to be resolved by a lowercase 40-hex commit ancestral to the final proof SHA and a manifest-covered receipt proving all three A/B scenarios succeeded at that commit with resolved parity. A dormant selector, unresolved gap, or free-form decision environment value is not completion.

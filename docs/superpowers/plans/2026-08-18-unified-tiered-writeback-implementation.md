...
- [ ] C7B records independent typed HPN-receive-win and ZeroFS-upload-gap flags in one manifest-validated archived authority; C8 consumes those flags with `ledger-value`/`validate-owned-path`, packages every HPN win including the combined outcome, and lands/reruns any measured upload correction. A dormant selector or free-form decision environment value is not completion.

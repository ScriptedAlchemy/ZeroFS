...
- [ ] C7B records independent typed HPN-receive-win, upload-gap-detected, and upload-gap-resolved flags in one manifest-validated archived authority; C8 requires every detected gap to be resolved with a 40-hex correction SHA and validated complete-rerun receipt, and packages every HPN win including the combined outcome. A dormant selector, unresolved gap, or free-form decision environment value is not completion.

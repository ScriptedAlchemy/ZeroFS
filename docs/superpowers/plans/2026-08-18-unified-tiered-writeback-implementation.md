...
- [ ] Sparse/interleaved reclaim verification for a full 1,024-frame segment uses at most 16 fixed 64-key batches across two views (32 scans total), scans at most 4,096 rows/64 MiB encoded bytes, has focused exact GREEN gates, and fails closed on row/byte exhaustion, scan/decode error, or live reference.

...
pub(crate) use failover::watch_heartbeats_until_takeover_hint;
...
#[allow(unused_imports)]
pub(crate) use tail::{ReplOp, TailBuffer};
...
const AUTHORITY_TTL: Duration = Duration::from_secs(3);
/// Active ownership validation interval for degraded and Solo owners.
const OWNERSHIP_VALIDATION_INTERVAL: Duration = Duration::from_secs(1);
/// Maximum time allowed for one ordinary ownership-record read. Serving remains
/// independently bounded by [`AUTHORITY_TTL`].
const OWNERSHIP_VALIDATION_TIMEOUT: Duration = Duration::from_secs(3);
/// Coverage/liveness beacon cadence.
pub(crate) const COVERAGE_HEARTBEAT_INTERVAL: Duration = Duration::from_millis(100);
/// Age after which standby acknowledgements no longer suppress ownership validation.
const HEARTBEAT_ACK_FRESH_FOR: Duration = COVERAGE_HEARTBEAT_INTERVAL.saturating_mul(3);
/// Heartbeat gap before attempting a durable claim.
pub(crate) const TAKEOVER_HINT_AFTER: Duration = Duration::from_secs(2);
/// Timeout for final ownership validation after authority expiry.
const FINAL_OWNERSHIP_RECOVERY_TIMEOUT: Duration = Duration::from_secs(3);
/// Bound for protocol and serving shutdown after authority loss.
pub(crate) const RESPONSE_DRAIN_TIMEOUT: Duration = Duration::from_secs(2);
...
impl ReplicationParams {
    pub(crate) fn from_config(cfg: &crate::config::ReplicationConfig) -> Self {
        Self {

...
/// Marks a create for an epoch/counter segment key generated by this store.
///
/// Writeback may admit these creates without consulting the remote store. It
/// still publishes them with create-only semantics and verifies any object that
...
impl SegmentStore {
    ...
    pub async fn seal_compressed(
        ...
        self.seal_compressed_with_limits(frames, SegmentFormatLimits::WIRE)
            .await
    }

    async fn seal_compressed_with_limits(
        &self,
        frames: Vec<(InodeId, u64, Compressed)>,
...
mod tests {
    ...
    #[tokio::test]
    async fn seal_rejects_format_overflow_before_allocating_id_or_publishing() {
        ...
        let err = store
            .seal_compressed_with_limits(frames, SegmentFormatLimits::with_u32_max(1))
            .await
            .unwrap_err();
    ...
    async fn single_put_segment_refuses_to_overwrite_an_existing_segment() {
        ...
        store
            .put_segment(segid, Bytes::from_static(b"first segment"))
            .await
            .unwrap();
        store
            .put_segment(segid, Bytes::from_static(b"replacement segment"))
            .await
            .expect_err("an immutable segment key must not be overwritten");

...
impl Settings {
    ...
    pub(crate) fn render_default_config() -> Result<String> {
        ...
        toml_string.push_str("# transport = \"russh\"            # native in-process SSH\n");
...
min_free_gb = 256.0"#,
    ...
    fn sftp_defaults_are_strict_and_bounded() {
        ...
        assert!(
            ...
            "generated config must document native russh"
        );
        assert!(
            !rendered.contains("hpn"),
            "generated config must not advertise HPN"
        );
    }

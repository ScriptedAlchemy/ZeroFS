...
async fn remote_client_with_read_disconnect() -> (
    ...
    tokio::spawn(async move {
        ...
        // This proxy intentionally permits only the established connection.
        // Once its armed Tread is observed, reconnect has no successor.
        drop(listener);
        let upstream = loop {
...
async fn assert_first_cancellation_interrupts_read(read_number: usize) {
    ...
    // Models the first Ctrl-C. No second cancellation should be needed to
    // interrupt a read whose only transport has disappeared.
    cancellation.cancel();

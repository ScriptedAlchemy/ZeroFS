...
Linux servers discover a finite `memory.max` on their current cgroup at startup. That limit must belong to the ZeroFS service; a shared ancestor is only a ceiling and is never treated as process-owned capacity. Container runtimes can hide a dedicated service limit from the process; declare that administrator-assigned envelope explicitly in that case:
...
`memory_limit_gb` is a decimal-GB ZeroFS-dedicated envelope, like the other `*_gb` settings. When both it and a current-cgroup limit are available, ZeroFS uses the smaller dedicated value. A finite shared ancestor may reject a declared budget that cannot fit, but it cannot authorize startup by itself. If only a shared ancestor is visible and no dedicated value is configured, startup fails closed. Platforms without cgroup v2 retain compatibility by logging that admission is unavailable unless `runtime.memory_limit_gb` is configured.

Before opening the storage backend or starting listeners, ZeroFS checks the resolved clean-cache, object-writeback, and volatile-write budgets against that envelope. The defense-in-depth admission guard also reserves 32 GiB for the unmodeled excess residency reproduced during the production incident and 8 GiB for GC, protocol companions, and the rest of the process. The same validated envelope, minus those reserves, becomes the process RSS pressure-brake threshold; downstream cache and GC code do not rediscover container limits independently. These conservative policy reserves do not prove every NFS, GC, prefetch, or allocator path is bounded; startup fails closed when the modeled tiers plus both reserves do not fit.
...
# Optional all-client identity for a trusted shared export.
[servers.nfs.shared_identity]
uid = 501
gid = 20
```

NFS is TCP-only and has no `unix_socket` key. By default, ZeroFS applies POSIX
permissions to the numeric UID, primary GID, and supplementary groups supplied
by each NFS client. When `shared_identity` is present, both `uid` and `gid` are
required and every client, including root, acts as that one identity. Creation
requests cannot override the configured owner, and later NFS chown/chgrp
requests are ignored. This mode removes per-client identity separation; use it
only on a private listener restricted to trusted clients. It does not rewrite
ownership or permissions on existing files.
...
write_ack_mode = "materialized"       # Safe default
# Dangerous, opt-in RAM acknowledgement for high-speed bursts:
# write_ack_mode = "volatile_memory"
# volatile_memory_gb = 8.0
```

`volatile_memory` replies to an ordinary NBD WRITE once ZeroFS owns the bytes
in a process-wide bounded RAM overlay. Those acknowledged bytes are lost on a
process crash or power failure until a FLUSH or FUA request completes. FLUSH
and FUA wait for canonical materialization and the normal durability barrier.
The RAM budget is shared by all NBD exports and backpressures writers when it
fills. This mode requires writable exclusive NBD access: replication, NFS, 9P,
WebUI, read-only/checkpoint operation, and `ignore_fsync` are rejected.
